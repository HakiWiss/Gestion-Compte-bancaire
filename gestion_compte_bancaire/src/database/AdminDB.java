...
public class AdminDB {
    ...
    public AdminDB(Connection connection) {
    ...
    public ArrayList<Bank> getBanks(int client_id) throws SQLException {
        String query = "SELECT * FROM Bank";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            try (ResultSet resultSet = statement.executeQuery()) {
                ArrayList<Bank> lstBanks=new ArrayList<>();
                if (resultSet.next()) {
                        lstBanks.add(new Bank(resultSet.getString(1), resultSet.getString(2)));
                }return lstBanks;
                } 
            }
        }
    ...
    public void insertCompte(Compte compte) throws SQLException {
        ...
        if (!query_check.checkCompte_exist(connection, compte.getRib())) {
        ...
        }else{System.out.println("Cette compte est deja cree par cet rib:"+compte.getRib());}
