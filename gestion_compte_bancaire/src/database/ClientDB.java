...
public class ClientDB {
    ...
    public Releve deposerArgent(Compte compte, float montant) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            if (rowsUpdated > 0) {
                ...
                try (PreparedStatement statement2 = connection.prepareStatement(releveQuery,
            ...
            try (ResultSet resultSet = checkStatement.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    if (solde >= montant) {
                        ...
                        System.out.println("le Montant est plus grand que le solde");
                    }
                }
            }
        }
    ...
    public boolean TranserfArgent(Compte compteSrc, Compte compteDes, float montant) throws SQLException {
        String checkbalancequery = "SELECT solde FROM Compte WHERE rib = ?;";
        String retirerquery = "UPDATE Compte SET solde = solde - ? WHERE rib = ?;";
        String deposirquery = "UPDATE Compte SET solde = solde + ? WHERE rib = ?;";
        String TransfertQuery = "INSERT INTO Transfert(date,montant,compte_src_id,compte_des_id) values(?,?,?,?);";
        try (PreparedStatement checkStatement = connection.prepareStatement(checkbalancequery)) {
            checkStatement.setString(1, compteSrc.getRib());
            try (ResultSet resultSet = checkStatement.executeQuery()) {
                if (resultSet.next()) {
                    float solde = resultSet.getFloat("solde");
                    if (solde >= montant) {
                        try (PreparedStatement statement = connection.prepareStatement(retirerquery)) {
                            statement.setFloat(1, montant);
                            statement.setString(2, compteSrc.getRib());
                            int rowsUpdated = statement.executeUpdate();
                            if (rowsUpdated > 0) {
                                try (PreparedStatement statement2 = connection.prepareStatement(deposirquery)) {
                                    statement2.setFloat(1, montant);
                                    statement2.setString(2, compteDes.getRib());
                                    int rowsUpdated2 = statement2.executeUpdate();
                                    if (rowsUpdated2 > 0) {
                                        java.sql.Date today = new java.sql.Date(System.currentTimeMillis());
                                        try (PreparedStatement statement3 = connection.prepareStatement(TransfertQuery)) {
                                            statement3.setDate(1, today); 
                                            statement3.setFloat(2, montant);
                                            statement3.setInt(3, compteSrc.getId());
                                            statement3.setInt(4, compteDes.getId());
                                            statement3.executeUpdate();
                                            int rowsUpdated3 = statement2.executeUpdate();
                                            if(rowsUpdated3>0){return true;}
                                        }
                                    }
                                }
                            }
                        ...
                        System.out.println("le Montant de transfert est plus grand que le solde");
                    }
                }
            }
        }
